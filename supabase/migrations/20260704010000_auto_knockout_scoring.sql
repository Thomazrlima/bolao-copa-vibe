...
do $$
begin
  if to_regprocedure('public.recalcular_pontuacao_chaveamento()') is not null then
    revoke execute on function public.recalcular_pontuacao_chaveamento()
    from public, anon, authenticated;
  end if;

  if to_regprocedure('public.recalcular_pontuacao_chaveamento_admin()') is not null then
    revoke execute on function public.recalcular_pontuacao_chaveamento_admin()
    from public, anon, authenticated;
  end if;
end;
$$;
...
drop trigger if exists observar_jogo_mata_mata_encerrado on public.jogos;
drop function if exists public.observar_jogo_mata_mata_encerrado();
...
begin
  ...
  users_count := public.recalcular_totais_usuarios();
