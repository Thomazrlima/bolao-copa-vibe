...
export const Route = createFileRoute("/grupos")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "12 grupos da Copa de 48 seleções com classificação em tempo real e ranking dos 3º colocados." },
      { property: "og:title", content: "Grupos · Bolão dos v(devers)" },
      { property: "og:description", content: "Classificação ao vivo dos grupos A a L e melhores terceiros colocados." },
    ],
  }),
...
function GruposPage() {
  ...
  return (
    <AppShell>
      <div className="mb-6">
        <h2 className="font-display text-2xl font-black tracking-tight sm:text-3xl">Grupos & Classificação</h2>
        <p className="text-sm text-muted-foreground">12 grupos · 4 seleções · 2 primeiros + 8 melhores 3ºs avançam</p>
      </div>
      ...
      <section className="mt-10">
        <div className="mb-3 flex items-end justify-between">
          <div>
            <h3 className="font-display text-xl font-black tracking-tight">Ranking dos 3º Colocados</h3>
            <p className="text-sm text-muted-foreground">8 melhores avançam ao mata-mata · atualização em tempo real</p>
          </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card">
          <div className="grid grid-cols-[36px_minmax(0,1fr)_40px_40px_40px_90px] gap-2 border-b border-border bg-background/40 px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground sm:grid-cols-[48px_minmax(0,1fr)_60px_60px_60px_120px] sm:px-5">
            <span>#</span><span>Seleção</span><span className="text-right">P</span><span className="text-right">SG</span><span className="text-right">GP</span><span className="text-right">Status</span>
          </div>
                ...
                >
                  ...
                  <span className="text-right">
                    <span className={cn(
                      ...
                      classified ? "bg-primary text-primary-foreground" : "bg-destructive/20 text-destructive",
                    )}>
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-card">
      <div className="flex items-center justify-between border-b border-border bg-background/40 px-4 py-2.5">
        ...
        <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">P · SG · GP</span>
      </div>
      <ul className="divide-y divide-border">
        {standings.map((s, i) => {
          const top2 = i < 2;
          ...
          return (
            ...
            >
              ...
              <span className="num text-right font-bold">{s.points}</span>
              <span className="num text-right text-muted-foreground">{s.gd > 0 ? `+${s.gd}` : s.gd}</span>
